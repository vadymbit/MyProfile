...
class ContactRepositoryImpl @Inject constructor(
    private val storage: UsersLocalStorage,
    private val apiService: ApiService,
    private val profStorage: AuthLocalStorage
) : ContactRepository, BaseRepository() {

    override suspend fun addContact(contact: UserModel): Result<Boolean, Exception> {
    ...
    override suspend fun removeContact(contactId: Int): Result<Boolean, Exception> {
    ...
    override suspend fun getAllUsers(): Result<List<UserModel>, Exception> {
        ...
        return getResult(apiResult) {
            (apiResult as ApiResult.Success).data.users.map {
                UserDataMapper.toUserModel(it)
            }
        }
    }

    override suspend fun getUserContacts(): Result<Flow<List<UserModel>>, Exception> {
    ...
    private suspend fun saveContactsToCache(contacts: List<UserDTO>) {
        storage.addContacts(contacts.map { userDTO ->
            UserDataMapper.toUserDB(userDTO)
        })
    }
